...
input_sample = pd.DataFrame({'Timestamp': pd.Series(['2003-05-01T00:00:00.000Z'], dtype='datetime64[ns]'), 'Holiday & Weekends': pd.Series(['0'], dtype='int64'), 'Price_Bruce': pd.Series(['25.42'], dtype='float64'), 'BRU_Temperature': pd.Series(['5.7'], dtype='float64'), 'BRU_Dew Point': pd.Series(['4.5'], dtype='float64'), 'BRU_Relative Humidity': pd.Series(['92.0'], dtype='float64'), 'BRU_Wind Speed': pd.Series(['9.0'], dtype='float64')})
...
def init():
    ...
    model_path = Model.get_model_path(model_name = 'AutoMLa9ded8aa537')
