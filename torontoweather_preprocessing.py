...
for i in range (2003, 2013): #repeats for data from 2003-2020
    ...
    with open('C:\\Users\\Adam\\Documents\\GitHub\\Sustainable-AI-Challenge\\Postprocessed_Dataset\\toronto_' + str(i) +'.csv', mode='w',newline='') as toronto_file:
        toronto_writer = csv.writer(toronto_file, delimiter=',',quoting=csv.QUOTE_ALL)
        toronto_writer.writerow(("Year","Month","Day","Hour","Temperature","Dew Point","Relative Humidity","Wind Speed",'\n')) #writes header
        next(csvtoronto)
        for row in csvtoronto:
